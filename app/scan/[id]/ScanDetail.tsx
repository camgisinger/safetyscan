...
async function exportScanPDF(scan: Scan, siteName: string | null, checklist: any[], checklistState: Record<string, any>, notes: string) {
  ...
  if (photoUrl) { const dataUrl = await imageUrlToDataUrl(photoUrl); if (dataUrl) { const dims = await new Promise<{w:number;h:number}>(res => { const img = new window.Image(); img.onload = () => res({w:img.naturalWidth,h:img.naturalHeight}); img.onerror = () => res({w:4,h:3}); img.src = dataUrl }); const maxW = cw, maxH = 90, r = dims.w / dims.h; const imgW = r > maxW / maxH ? maxW : maxH * r; const imgH = r > maxW / maxH ? maxW / r : maxH; try { doc.addImage(dataUrl, 'JPEG', ml, y, imgW, imgH) } catch (_) {}; y += imgH + 6 } }
